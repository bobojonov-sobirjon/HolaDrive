...
class UserAvatarUpdateView(AsyncAPIView):
    ...
    @swagger_auto_schema(
        tags=['User'],
        operation_description="Update profile picture (avatar). Send multipart/form-data with field 'avatar' (image file).",
        request_body=openapi.Schema(
            type=openapi.TYPE_OBJECT,
            required=['avatar'],
            properties={
        ...
        responses={
            ...
            400: openapi.Response(description="Bad request - avatar file required or invalid"),
            401: openapi.Response(description="Unauthorized"),
        }
    )
    async def put(self, request):
